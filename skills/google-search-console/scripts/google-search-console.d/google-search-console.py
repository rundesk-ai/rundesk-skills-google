...
def read_frame(connection: socket.socket, deadline: float) -> dict[str, Any]:
    ...
    if payload.get("ok") is not True:
        raise SearchConsoleError("Rundesk refused the Google request.")
    return payload
...
def refused(code: int, said: str, profile: str, trouble: str = "") -> SearchConsoleError:
...
def cmd_profiles(args: argparse.Namespace) -> None:
    ...
    if args.auth:
        sign_in(profile)
    write_rows(managed_rows(profile), ["profile", "account", "status"], args.json)
